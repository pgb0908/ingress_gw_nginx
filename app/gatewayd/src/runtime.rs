use crate::models::{DeployResult, LoadResult, MetricsState, PathMatchType, ReloadStatus, RuntimeState, ValidationResult, ValidationSnapshot};
use crate::nginx::{self, NginxManager};
...
pub fn load_state() -> Result<RuntimeState> {
    let path = nginx::state_file();
    if !path.exists() {
        return Ok(RuntimeState { metrics: MetricsState::default(), ..RuntimeState::default() });
    }
    ...
    pub fn deploy_resource(&self, body: &[u8]) -> Result<DeployResult, DeployError> {
        ...
        match self.nginx.activate(Path::new(conf_path)) {
            Ok(message) => {
                ...
                save_state(&state)?;
                Ok(DeployResult { kind, name, status: "applied".to_string(), message, validation: Some(validation) })
            }
            Err(error) => {
                state.metrics.gateway_reload_failures_total += 1;
                state.last_reload_status = Some(ReloadStatus { success: false, message: error.to_string() });
                save_state(&state)?;
                Ok(DeployResult { kind, name, status: "failed".to_string(), message: error.to_string(), validation: Some(validation) })
            }
        }
    }
    ...
    pub fn load_revision(&self, revision_path: &Path) -> Result<LoadResult> {
        ...
        match self.nginx.activate(Path::new(conf_path)) {
            Ok(message) => {
                ...
                state.last_reload_status = Some(ReloadStatus { success: true, message: message.clone() });
                save_state(&state)?;
                Ok(LoadResult { revision: Some(validation.revision.clone()), status: "loaded".to_string(), message, validation: Some(validation) })
            }
            Err(error) => {
                state.metrics.gateway_reload_failures_total += 1;
                state.last_reload_status = Some(ReloadStatus { success: false, message: error.to_string() });
                save_state(&state)?;
                Ok(LoadResult { revision: Some(validation.revision.clone()), status: "reload_failed".to_string(), message: error.to_string(), validation: Some(validation) })
            }
        }
    }
}
...
pub fn validate_bundle(revision_path: &Path, nginx: &NginxManager) -> Result<ValidationResult> {
    ...
    for plugin in &bundle.manifest.plugins {
        ...
        if plugin.version.starts_with("0.") {
            warnings.push(format!("plugin {} uses pre-1.0 version {}", plugin.name, plugin.version));
        }
    }
            ...
            PathMatchType::Regex => {
                ...
                for rule in &router.spec.rules {
                    if let Err(e) = regex::Regex::new(&rule.path) {
                        errors.push(format!("Router '{name}': invalid regex '{}': {e}", rule.path));
                    }
                }
            }
        }
